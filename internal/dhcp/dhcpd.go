...
func groupCIDR(g model.Group, opCode byte) (int, error) {
	cidr, err := netutil.NetmaskToCIDR(g.Netmask)
	if err != nil {
		logrus.WithFields(logrus.Fields{
			"opcode":  opCode,
...
func groupGateway(g model.Group, opCode byte) (net.IP, error) {
...
func AddOptions(req *layers.DHCPv4, resp *layers.DHCPv4, lease *model.Host, ip net.IP) error {
	...
	byOpCode := make(map[byte][]model.Option)
	...
	var h model.Host
	if err := store.DB.Preload("Group").First(&h, lease.ID).Error; err != nil {
	...
	for opCode := range requestedOptions {
		...
		case layers.DHCPOptBroadcastAddr:
			...
			bcastStr, err := netutil.BroadcastAddress(h.Group.Gateway, strconv.Itoa(cidrMask))
			if err != nil {
				logrus.WithFields(logrus.Fields{
					"opcode": opCode,
					"name":   layers.DHCPOpt(opCode).String(),
		...
		case layers.DHCPOptT1:
			resp.Options = append(resp.Options, model.NewUint32Option(layers.DHCPOptT1, int(leaseTime*0.5))) // renewal time
		case layers.DHCPOptT2:
			resp.Options = append(resp.Options, model.NewUint32Option(layers.DHCPOptT2, int(leaseTime*0.875))) // rebind time
		case layers.DHCPOptLeaseTime:
			resp.Options = append(resp.Options, model.NewUint32Option(layers.DHCPOptLeaseTime, int(leaseTime))) // lease time
...
func Init(intf string) {
	...
	//64bit x86 UEFI PXE
	var x86_64_pxe model.DeviceClass
	if res := store.DB.FirstOrCreate(&x86_64_pxe, model.DeviceClass{DeviceClassForm: model.DeviceClassForm{Name: "PXE-UEFI_x64", VendorClass: "PXEClient:Arch:00007"}}); res.Error != nil {
		logrus.Warning(res.Error)
	}

	//64bit x86 UEFI HTTP
	var x86_64_http model.DeviceClass
	if res := store.DB.FirstOrCreate(&x86_64_http, model.DeviceClass{DeviceClassForm: model.DeviceClassForm{Name: "HTTP-UEFI_x64", VendorClass: "HTTPClient:Arch:00016"}}); res.Error != nil {
		logrus.Warning(res.Error)
	}

	//64bit ARM UEFI PXE
	var arm_64_pxe model.DeviceClass
	if res := store.DB.FirstOrCreate(&arm_64_pxe, model.DeviceClass{DeviceClassForm: model.DeviceClassForm{Name: "PXE-UEFI_ARM64", VendorClass: "PXEClient:Arch:00011"}}); res.Error != nil {
		logrus.Warning(res.Error)
	}

	//64bit ARM UEFI HTTP
	var arm_64_http model.DeviceClass
	if res := store.DB.FirstOrCreate(&arm_64_http, model.DeviceClass{DeviceClassForm: model.DeviceClassForm{Name: "HTTP-UEFI_ARM64", VendorClass: "HTTPClient:Arch:00011"}}); res.Error != nil {
		logrus.Warning(res.Error)
	}
...
func findHostByMAC(mac string) (*model.Host, error) {
	var host model.Host
	res := store.DB.
		Preload("Group").
