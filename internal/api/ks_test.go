...
// the host.
func seed(t *testing.T, group model.Group, host model.Host) *model.Host {
	t.Helper()
	...
	if err := store.DB.AutoMigrate(&model.Host{}, &model.Option{}, &model.DeviceClass{}, &model.Group{}, &model.Image{}, &model.User{}); err != nil {
...
func TestKsClearsTheReimageFlag(t *testing.T) {
	...
	var after model.Host
	if res := store.DB.First(&after, host.ID); res.Error != nil {
