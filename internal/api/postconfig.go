...
// Post-configuration is done by the host itself, in the %firstboot section of
// the kickstart: NTP, syslog, SSH, the FQDN and the TLS certificate are all
// configured there. This file only records that the host finished and fires
// the group's callback URL.
//
// It used to work the other way round, with the appliance polling the new
// host's SOAP API over govmomi for up to an hour using its root credentials.
// That code had already been commented out before this fork; removing it drops
// the govmomi dependency entirely.
